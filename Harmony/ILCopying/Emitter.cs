...
namespace HarmonyErdelf.ILCopying
{
	public static class Emitter
	{
		...
		{
            int offset = Traverse.Create(il).Field("code_len").GetValue<int>();
		...
		public static void LogLastLocalVariable(ILGenerator il)
		{
			...
			{
                LocalBuilder[] existingLocals = Traverse.Create(il).Field("locals").GetValue<LocalBuilder[]>();
				if (existingLocals.Length > 0)
				{
                    LocalBuilder latestVar = existingLocals.Last();
